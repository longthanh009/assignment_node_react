...
import { read, updateOrder } from '../../api/orderApi';
...
const CartPage = () => {
    ...
    useEffect(() => {
        const getOrders = async () => {
            if (user) {
                const { data } = await read(user._id);
                console.log(data);
                setOrders(data)
            }
        }
    ...
    const handlerSubmitCancle = async (_id: any) => {
        const { data } = await updateOrder(_id, { status: 3 });
        setOrders(orders.map(item => item._id === _id ? data : item))
    }
    const onHandlerRefet = async (_id: any) => {
        const { data } = await updateOrder(_id, { status: 0 });
        setOrders(orders.map(item => item._id === _id ? data : item))
    };
    const getDetailOrder = async (_id) => {
        const { data } = await orderDetailList(_id);
        return data
    }
    return (
        <div>
            ...
            <div>
                <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
                    <div className="flex justify-start item-start space-y-2 flex-col">
                        <h1 className="text-3xl dark:text-white lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">Your order</h1>
                        <p className="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">All that you have booked</p>
                    </div>
                            ...
                            <div key={index + 1
                                ...
                                <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                                    <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                                        <p className="mb-5 text-lg md:text-xl dark:text-white font-semibold leading-6 xl:leading-5 text-gray-800">
                                            # {item._id} / <span className='text-green-600'>
                                                {item.status === 0 ? "Chưa xác nhận" : ""} {item.status === 1 ? "Đã gửi hàng" : ""} {item.status === 2 ? "Giao hàng thành công" : ""}
                                                {item.status === 3 ? "Đã huỷ" : ""}</span>
                                        </p>
                                    ...
                                    <div className="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
                                        <div className="flex flex-col justify-start items-start flex-shrink-0">
                                            <div className="flex justify-center text-gray-800 dark:text-white md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                                                ...
                                                <p className="cursor-pointer text-sm leading-5 ">0{item.phone}</p>
                                            </div>
                                        </div>
                                        <div className="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
                                            <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                                                <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                                                    <p className="text-base dark:text-white font-semibold leading-4 text-center md:text-left text-gray-800">Recipient's name</p>
                                                    <p className="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{item.name}</p>
                                                </div>
                                                <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
                                                    ...
                                                    <p className="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{item.address}</p>
                                                </div>
                                            </div>
                                            <div className="flex w-full justify-center items-center md:justify-start md:items-start">
                                                {item.status < 1 ? <button
                                                    onClick={() => {
                                                        handlerSubmitCancle(item._id)
                                                    }}
                                                    className="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base font-medium leading-4 text-gray-800">Cancel</button> : ""}
                                                {item.status === 3 ? <button
                                                    onClick={() => {
                                                        onHandlerRefet(item._id)
                                                    }}
                                                    className="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base font-medium leading-4 text-gray-800">Repurchase</button> : ""}
                                            </div>
                                        </div>
                                    </div>
                                </div>
